...
| ジョブ | 結果 |
| --- | --- |
| 閉包の脆弱性スキャン | pass / 2 分 45 秒（閉包と grype DB のダウンロードを含む） |
| pin と先端の閉包差分 | pass / 6 秒（この PR は `nix/flake.lock` を動かしていないので、スキップ側の分岐が確認できた） |
| 既存ジョブ（flake check ×2 / lock-age / lint） | すべて pass（退行なし） |
